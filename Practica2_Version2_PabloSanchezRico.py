...
class Monitor():
    ...
    def turn_car_north(self):
        return self.turn.value == 1

    def turn_car_south(self):
        return self.turn.value == 0

    def turn_ped(self):
        return self.turn.value == 2
    ...
    def wants_enter_car(self, direction: int) -> None:
        ...
        if direction==SOUTH:
            ...
            if self.turn.value == -1:
                self.turn.value = 0
            self.open_south.wait_for(self.turn_car_south) and self.open_south.wait_for(self.no_cars_north) and self.open_south.wait_for(self.no_pedestrian)
        ...
        if direction==NORTH:
            ...
            if self.turn.value == -1:
                self.turn.value = 1
            self.open_north.wait_for(self.turn_car_north) and self.open_north.wait_for(self.no_cars_south) and self.open_north.wait_for(self.no_pedestrian)
    ...
    def leaves_car(self, direction: int) -> None:
        ...
        if direction==SOUTH:
            self.inside_south.value -= 1
            if self.turn == 0:
                if self.Wcar_north.value > 0:
                    self.turn.value = 1
                elif self.Wpedestrian.value > 0:
                    self.turn.value = 2
                elif self.Wcar_south.value == 0:
                    self.turn.value = -1
        ...
        if direction==NORTH:
            self.inside_north.value -= 1
            if self.turn == 1:
                if self.Wpedestrian.value > 0:
                    self.turn.value = 2
                elif self.Wcar_south.value > 0:
                    self.turn.value = 0
                elif self.Wcar_north.value == 0:
                    self.turn.value = -1
    ...
    def wants_enter_pedestrian(self) -> None:
        ...
        if self.turn.value == -1:
            self.turn.value = 2         
        self.open_pedestrian.wait_for(self.turn_ped) and self.open_pedestrian.wait_for(self.no_cars_north) and self.open_pedestrian.wait_for(self.no_cars_south)
