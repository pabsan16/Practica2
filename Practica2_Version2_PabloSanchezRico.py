...
class Monitor():
    ...
    def yes_cars_south(self):
        return self.turn.value == 0 and self.inside_north.value == 0 and self.inside_pedestrian.value == 0
        
    def yes_cars_north(self):
        return self.turn.value == 1 and self.inside_south.value == 0 and self.inside_pedestrian.value == 0
        
    def yes_pedestrian(self):
        return self.turn.value == 2 and self.inside_north.value == 0 and self.inside_south.value == 0
    ...
    def wants_enter_car(self, direction: int) -> None:
        ...
        if direction==SOUTH:
            ...
            if self.turn.value == -1:
                self.turn.value = 0
            self.open_south.wait_for(self.yes_cars_south)
        ...
        if direction==NORTH:
            ...
            if self.turn.value == -1:
                self.turn.value = 1
            self.open_north.wait_for(self.yes_cars_north)
    ...
    def wants_enter_pedestrian(self) -> None:
        ...
        if self.turn.value == -1:
            self.turn.value = 2         
        self.open_pedestrian.wait_for(self.yes_pedestrian)
    ...
    def leaves_pedestrian(self) -> None:
        ...
        if self.turn == 2:
            if self.Wcar_south.value > 0:
                ...
                self.turn.value = -1
        if self.inside_pedestrian.value == 0:
            self.open_north.notify_all()
            self.open_south.notify_all()
        self.mutex.release()
